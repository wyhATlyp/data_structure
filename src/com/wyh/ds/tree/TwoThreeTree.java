 ...
 *  思路：
 *  	1.构建二三四树：
 *  		判断结点类型 -> 比大小 -> 判断是否需放在子树上
 *  	2.分裂：最先分裂的一定是叶子结点，parent为null时该结点为根结点 u1 u2 u3分裂成u2(u1 u3)
 *  	3.重组：分裂后的父结点与原父结点整合
 */
public class TwoThreeTree {
	...
	/**
	 * 分裂
	 * 还需要区分当前结点类型为左子树、中子树、右子树
	 */
	public void split(Node parent, Node node) {
		if(parent == null || parent.u1 == null) {//父结点零结点-根结点
			//分裂
	 ...
	 * 
	 * 临时的四结点：left u1 lCenter u2 rCenter u3 right
	 * 
	 * u3始终为空，当u3不为空时需分裂 left u1 lCenter u2 rCenter  u3 right
	 */
